...
pub enum Value {
    ...
    Number(Number),                    // u64 | i64 | f64 | BigInt — integers get arbitrary precision; fractions use f64
...
- Float comparison uses the `Number` type's `PartialEq` implementation, which
  compares `f64` values directly (`#![allow(clippy::float_cmp)]` in `number.rs`).
...
- Non-string `Object` keys are serialized by converting the key to a JSON string (via `serde_json::to_string`) and using that string as the JSON object key
- `Undefined` serializes as the string `"<undefined>"`
