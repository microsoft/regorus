...
""";
  ...
  ""properties"": {
    ...
    private const string HTTPS_DENY_DEFINITION = @"{
        ""policyRule"": {
            ""if"": {
                ""allOf"": [
    ...
    static void DemonstrateAzurePolicyJsonCompilation()
    {
        ...
        // 2. Compile the JSON policy definition directly (no Rego needed)
        using var program = Regorus.AzurePolicyCompiler.CompilePolicyDefinition(registry, HTTPS_DENY_DEFINITION);
        Console.WriteLine("Compiled Azure Policy JSON definition to RVM program");
