...
src/builtins/
  ...
  conversions.rs   # to_number, units.parse, units.parse_bytes
  encoding.rs      # base64, json, yaml, hex, urlquery, crypto
  glob.rs          # glob.match, glob.quote_meta
  graph.rs         # graph.reachable, graph.reachable_paths
  http.rs          # http.send
  net.rs           # net.cidr_contains, net.cidr_merge
...
fn my_builtin(span: &Span, params: &[Ref<Expr>], args: &[Value], strict: bool) -> Result<Value> {
    ...
    // Type-check arguments — strict mode raises errors, non-strict returns Undefined
    let arg0 = match &args[0] {
        Value::String(s) => s,
        a if strict => {
            let span = params[0].span();
            bail!(span.error(
                format!("`my_builtin` requires string argument. Got `{a}`.").as_str()
            ))
        }
...
Key patterns:
- **Return `Value::Undefined`** for type mismatches in non-strict mode (OPA semantics)
- **Return `Err`** for type mismatches in strict mode, and for genuine errors (wrong arg count, internal failure)
