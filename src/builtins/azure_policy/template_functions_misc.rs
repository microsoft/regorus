...
pub(super) fn register(m: &mut builtins::BuiltinsMap<&'static str, builtins::BuiltinFcn>) {
    ...
    // guid() and uniqueString() are not yet implemented.  They are only used
    // inside ARM deployment templates which the compiler skips (not evaluated
    // during policy compliance checks).
}
