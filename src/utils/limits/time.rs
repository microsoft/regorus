...
/// what deadline to enforce. A `None` limit disables time checks, which is the default state used by
/// new engines and the process-wide fallback installed via
/// [`set_global_execution_timer_config`]. Applications typically set a concrete limit along with a
/// small [`NonZeroU32`] interval so interpreter loops amortize their clock reads without skipping
/// checks for long stretches of repetitive work.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ExecutionTimerConfig {
    /// Maximum allowed wall-clock duration (None disables time checks).
    pub limit: Option<Duration>,
    /// Number of work units between time checks (minimum 1).
    pub check_interval: NonZeroU32,
}

impl Default for ExecutionTimerConfig {
    fn default() -> Self {
        Self {
            limit: None,
            check_interval: NonZeroU32::MIN,
        }
    }
}
...
static GLOBAL_EXECUTION_TIMER_CONFIG: Mutex<Option<ExecutionTimerConfig>> = Mutex::new(None);
...
///     global_execution_timer_config,
///     set_global_execution_timer_config,
...
/// };
/// set_global_execution_timer_config(Some(config));
/// assert_eq!(global_execution_timer_config(), Some(config));
/// ```
pub fn set_global_execution_timer_config(config: Option<ExecutionTimerConfig>) {
    *GLOBAL_EXECUTION_TIMER_CONFIG.lock() = config;
}
...
///
/// ```
/// use regorus::utils::limits::global_execution_timer_config;
///
/// // By default no global execution timer is configured.
/// assert!(global_execution_timer_config().is_none());
/// ```
pub fn global_execution_timer_config() -> Option<ExecutionTimerConfig> {
    let guard = GLOBAL_EXECUTION_TIMER_CONFIG.lock();
    guard.as_ref().copied()
}

impl ExecutionTimer {
    /// Construct a new timer with the provided configuration.
    pub const fn new(config: ExecutionTimerConfig) -> Self {
        Self {
            config,
            start: None,
            accumulated_units: 0,
    ...
    pub const fn config(&self) -> ExecutionTimerConfig {
        self.config
    }
    ...
    pub fn tick(&mut self, work_units: u32, now: Duration) -> Result<(), LimitError> {
        ...
        if self.accumulated_units < self.config.check_interval.get() {
            return Ok(());
        }

        // Preserve the remainder so that callers do not lose fractional work.
        let interval = self.config.check_interval.get();
    ...
    pub fn check_now(&mut self, now: Duration) -> Result<(), LimitError> {
        ...
        if elapsed > limit {
            return Err(LimitError::TimeLimitExceeded { elapsed, limit });
        }
        Ok(())
    }
...
mod tests {
    ...
    fn tick_defers_checks_until_interval_is_reached() {
        let mut timer = ExecutionTimer::new(ExecutionTimerConfig {
            limit: Some(Duration::from_millis(100)),
            check_interval: nz(4),
        });
    ...
    fn tick_reports_limit_exceeded() {
        let mut timer = ExecutionTimer::new(ExecutionTimerConfig {
            limit: Some(Duration::from_millis(30)),
            check_interval: nz(2),
        });
    ...
    fn check_now_allows_elapsed_equal_to_limit() {
        let mut timer = ExecutionTimer::new(ExecutionTimerConfig {
            limit: Some(Duration::from_millis(50)),
            check_interval: nz(1),
        });
    ...
    fn check_now_is_noop_before_start() {
        let mut timer = ExecutionTimer::new(ExecutionTimerConfig::default());
